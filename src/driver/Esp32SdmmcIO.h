...
class Esp32SdmmcIO : public BaseIO {
 public:
  ...
  Esp32SdmmcIO(bool mode1bit, int max_freq_khz = SDMMC_FREQ_DEFAULT) 
      : card(nullptr), auto_init(true), init_mode1bit(mode1bit), 
   ...
   * or you can call begin() explicitly.
   */
  Esp32SdmmcIO(bool mode1bit, int max_freq_khz,
                 const sdmmc_host_t& host_cfg, 
                 const sdmmc_slot_config_t& slot_cfg)
      : card(nullptr), auto_init(true), init_mode1bit(mode1bit), 
        init_max_freq_khz(max_freq_khz), host_config(host_cfg), 
  ...
  bool begin(bool mode1bit = false, int max_freq_khz = SDMMC_FREQ_DEFAULT) {
    return begin(mode1bit, max_freq_khz, 
                 SDMMC_HOST_DEFAULT(), 
   ...
   * @return true if initialization succeeded, false otherwise
   */
  bool begin(bool mode1bit, int max_freq_khz,
             sdmmc_host_t host_cfg, sdmmc_slot_config_t slot_cfg) {
  ...
  uint32_t getFreqKHz() const {
    return card ? card->max_freq_khz : 0;
  }
